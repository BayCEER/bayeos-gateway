...
@Component
@Profile("default")
class ExportJobXmlRpc implements Runnable  {
	
	private Logger log = LoggerFactory.getLogger(ExportJobXmlRpc.class)
	...
	public void run() {
		try {
			...
			while(true){
				...
				try {
					log.info("ExportJobXmlRpc running")
				...
				log.info("ExportJobXmlRpc finished")
				Thread.sleep(1000*expWaitSecs)
			}
	...
	private def exportNewObservations() {
		...
		try {
			while (true){
				def row = 0
				def dbSeriesId = []
				db.eachRow("select id,db_series_id from channel order by id;"){ r ->
					dbSeriesId[r.id] = r.db_series_id; 
				}
				
				// Read Stream on Redis and send it to XmlRpc
	...
	public void start(){
		if (redis != null) {
			...
			log.info("ExportJobXmlRpc started")
		}
	}
